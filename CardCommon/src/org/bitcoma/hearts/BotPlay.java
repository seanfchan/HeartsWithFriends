...
public class BotPlay
{
	...
	public BotPlay(LinkedList<Card> cards, boolean skipInit)
	{
		...
		// TODO @madiha: Wait to compute probabilities until the bot has received all three card.
		
		// we would skip initialization if the bot replaced a person in the middle of the game, as then
	...
	public void removeThree()
	{
		 ...
		 * 1. Definitely removes the Queen of Spades if it has that.
		 * 2. Checks how many cards of each suit it has and makes sure to have as much suit variety
		 * as possible.
		 * 3. Remove high cards from the suit.
		 */
		Iterator<Card> cardIter = matrix.keySet().iterator();
		while (cardIter.hasNext())
		{
			...
			if (considered.getSuit() == Card.SPADES && considered.getRank() == Card.QUEEN)
			{
				...
				matrix.remove(considered); //removed queen of spades from the matrix.
				break;
			}
		}
		
		
		// Discard the card of the most represented suit and one that has the highest value.
	...
	public void initialize()
	{
        ...
        // each card get a probability proportional to its rank regardless of the suit.
        ...
        // add back if the queen of spades and two of clubs were removed based on the boolean
        if (hasQueenSpades)
        {
	...
	{
		// based on the existing cards that the bot has, it will decide what it can play. 
	...
	{
		// re-normalizing probabilites based on memory and cards remaining. Sort of like in initialize
	}
